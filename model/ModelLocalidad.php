...
class ModelLocalidad
{
	...
	{
		$sql = $this->base_datos->query("SELECT * FROM localidades WHERE zona_id='$zonaId' AND status='0' ORDER by ciudad")->fetchAll(PDO::FETCH_ASSOC);
		return $sql;
	}


	function listaPorMunicipio($municipioId)
	{
		$sql = $this->base_datos->query("SELECT * FROM localidades WHERE municipio_id='$municipioId'")->fetchAll(PDO::FETCH_ASSOC);
		return $sql;
	}


	function obtenerPorNombre($nombre){
		$sql = $this->base_datos->get("localidades","*",
			["AND" => [
				"nombre[=]" => $nombre
			]
		]);
