<?php
	...
	if(empty($validarnota)){
		echo "<script> 
				...
				window.location.href = '../../view/creditosgasolina/mod_info_cred.php?numfac=".$notafac."&name=".$idcliente."';
			  </script>";
	}else{
									...
									if(strlen($litros)<1){
										...
										if(strlen($importe)<1){
											...
											if(strlen($vencimiento)<1){
												...
												$mode_credit->updatecreditogasolina($fecha,$nombre,$domicilio,$colonia,$notafac,$precio,$litros,$importe,$vencimiento,$id,$vendedor,$zona,$dia,$mes,$anio,$tipo_credito);
