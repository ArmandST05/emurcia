<?php
	...
	$precio = $_POST["pre"];
	$litros = $_POST["lit"];
	$importe = $_POST["imp"]; //Importe capturado en el campo importe (Pagado en esta ocacion)
	...
	$validarNota = $modelCredito->obtenerCreditoGasZonaFactura($zona,$notafac); 

	if(empty($validarNota)){
		echo "<script> 
				alert('La nota o factura ingresada no existe');
				window.location.href = '../../view/index.php?action=creditos/capturar_recuperado_cliente.php&cliente=".$idcliente."&numFactura=".$notafac."';
			  </script>";
	}else{
		if(strlen($fecha)<1){
			echo "<script> 
									...
									if(strlen($vencimiento)<1){
										...
										if(strlen($nombre)<1){
											...
											if(floatval($oldimpor) == floatval($importepagado)){
												$modelCredito->updatestatus($idcliente,$notafac);
												$modelCredito->addabono($importepagado,$idcliente,$notafac);
												$modelCredito->addcreditrec($idcliente,$fecha,$nombre,$domicilio,$colonia,$notafac,$precio,$litros,$importepagado,$vencimiento,$vendedor,$zona,$dia,$mes,$anio);
