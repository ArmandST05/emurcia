...
if (!empty($valor)) {
  if($tipoBusqueda == "factura"){
    $creditos = $modelCredito->obtenerCreditosGasFactura($valor);
  }
  else{
    $creditos = $modelCredito->obtenerCreditosGasCliente($valor);
              ...
              <tbody>
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <button class='btn btn-sm btn-primary' type='button' onclick="eliminar('<?php echo $credito['idcreditogas']; ?>');"><i class='fas fa-trash fa-sm'></i></button>
                    </td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
...
<script type="text/JavaScript">
  ...
  function obtenerDatosCredito(){
    ...
    if(combo.selectedIndex<0){
     ...
     window.location.href = 'index.php?action=creditos/administrar_cliente.php&valor='+valor+'&tipoBusqueda='+tipoBusqueda;
    }
  }

  function editar(id) {
    window.location.href = 'index.php?action=creditos/editar.php&id='+id;
  }
  ...
  function eliminar(id) {
    alertify.confirm("¿Realmente desea eliminar el crédito seleccionado?",
      function() {
        $.ajax({
            type: "POST",
            url: "../controller/Creditos/EliminarCredito.php",
            data: {
              id: id
            },
            success: function(data) {
              location.reload();
              alertify.success("Crédito eliminado [Si existía un crédito recuperado de esta nota, fue eliminado]");
            }
          });
      },
      function() {
