...
if (!empty($idCliente)) {
  ...
  if ($tipoCredito == "otorgados") {
    $info = $modelCredito->obtenerCreditosOtorgadosGasolinaCliente($idCliente);
  } else if ($tipoCredito == "recuperados") {
    $info = $modelCredito->obtenerCreditosRecuperadosGasolinaCliente($idCliente);
    $abonos = $modelCredito->obtenerreportesabonos_cli($idCliente);
  } else if ($tipoCredito == "todos") {
    $info = $modelCredito->traercreditosclientegasolina($idCliente);
    $abonos = $modelCredito->obtenerreportesabonos_cli($idCliente);
  }
}
?>
  ...
  <div class="row">
    <div class="col-xl-12 col-lg-12">
      <div class="card shadow mb-4">
        <!-- Card Header - Dropdown -->
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
          <h6 class="m-0 font-weight-bold text-primary">Lista de créditos <?php echo $tipoCredito ?></h6>
        </div>
        <!-- Card Body -->
        <div class="card-body">
...
<script type="text/JavaScript">
  ...
  function obtenerDatosCredito(){
    ...
    if(combo.selectedIndex<0 || tipoBusqueda == ""){
     ...
     window.location.href = 'index.php?action=creditosgasolina/consultar_creditos.php&cliente='+idCliente+'&tipoBusqueda='+tipoBusqueda;
    }
  }
